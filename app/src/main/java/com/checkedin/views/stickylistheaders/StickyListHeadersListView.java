package com.checkedin.views.stickylistheaders;
...
import com.checkedin.views.stickylistheaders.WrapperViewList.LifeCycleListener;
...
public class StickyListHeadersListView extends FrameLayout {
    ...
    /* --- Children --- */
    private WrapperViewListOld mList;
    ...
    public StickyListHeadersListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        // Initialize the list
        mList = new WrapperViewListOld(context);
        // mList.setSelector(Color.TRANSPARENT);
        ...
        mList.setSelector(context.getResources().getDrawable(R.drawable.sticky_trans));
        mList.setLifeCycleListener(new WrapperViewListLifeCycleListener());
        mList.setOnScrollListener(new WrapperListScrollListener());
        addView(mList);

        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, Styleable.StickyListHeadersListView, 0, 0);

            try {
                // Android attributes
                if (a.hasValue(Styleable.StickyListHeadersListView_android_padding)) {
                    int padding = a.getDimensionPixelSize(Styleable.StickyListHeadersListView_android_padding, 0);
                    mPaddingLeft = padding;
                    mPaddingTop = padding;
                    mPaddingRight = padding;
                    mPaddingBottom = padding;
                } else {
                    mPaddingLeft = a.getDimensionPixelSize(Styleable.StickyListHeadersListView_android_paddingLeft, 0);
                    mPaddingTop = a.getDimensionPixelSize(Styleable.StickyListHeadersListView_android_paddingTop, 0);
                    mPaddingRight = a.getDimensionPixelSize(Styleable.StickyListHeadersListView_android_paddingRight, 0);
                    mPaddingBottom = a.getDimensionPixelSize(Styleable.StickyListHeadersListView_android_paddingBottom, 0);
                }
                setPadding(mPaddingLeft, mPaddingTop, mPaddingRight, mPaddingBottom);

                // Set clip to padding on the list and reset value to default on
                // wrapper
                mClippingToPadding = a.getBoolean(Styleable.StickyListHeadersListView_android_clipToPadding, true);
                super.setClipToPadding(true);
                mList.setClipToPadding(mClippingToPadding);

                // ListView attributes
                mList.setFadingEdgeLength(a.getDimensionPixelSize(Styleable.StickyListHeadersListView_android_fadingEdgeLength, mList.getVerticalFadingEdgeLength()));
                final int fadingEdge = a.getInt(Styleable.StickyListHeadersListView_android_requiresFadingEdge, 0);
                ...
                /*
				 * mList.setCacheColorHint(a .getColor( Styleable.StickyListHeadersListView_android_cacheColorHint, mList.getCacheColorHint()));
				 */
                mList.setCacheColorHint(Color.TRANSPARENT);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB)
                    mList.setChoiceMode(a.getInt(Styleable.StickyListHeadersListView_android_choiceMode, mList.getChoiceMode()));
                mList.setDrawSelectorOnTop(a.getBoolean(Styleable.StickyListHeadersListView_android_drawSelectorOnTop, false));
                mList.setFastScrollEnabled(a.getBoolean(Styleable.StickyListHeadersListView_android_fastScrollEnabled, mList.isFastScrollEnabled()));
                mList.setScrollBarStyle(a.getInt(Styleable.StickyListHeadersListView_android_scrollbarStyle, 0));
                final Drawable selector = a.getDrawable(Styleable.StickyListHeadersListView_android_listSelector);
                if (selector != null)
                    mList.setSelector(selector);
                mList.setScrollingCacheEnabled(a.getBoolean(Styleable.StickyListHeadersListView_android_scrollingCache, mList.isScrollingCacheEnabled()));
                ...
                // }
                // mDividerHeight =
                // a.getInt(R.styleable.StickyListHeadersListView_android_dividerHeight,
                // mDividerHeight);
                mDividerHeight = 0;

                // StickyListHeaders attributes
                mAreHeadersSticky = a.getBoolean(Styleable.StickyListHeadersListView_hasStickyHeaders, true);
                mIsDrawingListUnderStickyHeader = a.getBoolean(Styleable.StickyListHeadersListView_isDrawingListUnderStickyHeader, true);
    ...
    // This is called in response to the data set or the adapter_timeline_friend changing
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child instanceof WrapperView) {
                ...
                if (wrapperViewChild.hasHeader()) {
                    ...
                    } else if (childHeader.getVisibility() != View.VISIBLE)
    ...
    private class WrapperViewListLifeCycleListener implements LifeCycleListener, WrapperViewListOld.LifeCycleListener {
    ...
    public void setAdapter(StickyListHeadersAdapter adapter) {
        ...
        // if (adapter_timeline_friend instanceof SectionIndexer) {
        // mAdapter = new SectionIndexerAdapterWrapper(getContext(), adapter_timeline_friend);
        // } else
        {
    ...
    public static interface StickyListHeadersAdapter extends ListAdapter {
        /**
         ...
         *
         * @param position
         * The position of the item within the adapter_timeline_friend's data set of the item whose
         * header view we want.
         * @param convertView
         ...
         *
         * @param position
         * The position of the item within the adapter_timeline_friend's data set whose header id we
         * want.
         * @return
