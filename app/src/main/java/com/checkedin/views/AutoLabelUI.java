...
public class AutoLabelUI extends AutoViewGroup implements Label.OnClickCrossListener, Label.OnLabelClickListener {
	...
	private boolean isUnLimitedLables = true;
	...
	private void getAttributes(AttributeSet attrs) {
		...
		if (typedArray != null) {
			try {
				mTextSize = typedArray.getDimensionPixelSize(R.styleable.LabelsView_text_size, getResources().getDimensionPixelSize(R.dimen._14sdp));
				mTextColor = typedArray.getColor(R.styleable.LabelsView_text_color, getResources().getColor(android.R.color.white));
				mBackgroundResource = typedArray.getResourceId(R.styleable.LabelsView_label_background_res, R.color.white);
				...
				mLabelPadding = typedArray.getDimensionPixelSize(R.styleable.LabelsView_label_padding, getResources().getDimensionPixelSize(R.dimen._1sdp));
	 ...
	 *
	 * @param textLabel
	 *            is the text of the label added.
	 * @param position
	 *            is the position of the label using a list.
	 */
	public boolean addLabel(String textLabel, int position) {
		if (isUnLimitedLables || !checkLabelsCompleted()) {
			Label label = new Label(getContext(), mTextSize, mIconCross, mShowCross, mTextColor, mBackgroundResource, mLabelsClickables, mLabelPadding);
	 ...
	 *
	 * @param tag
	 *            is the tag of the label added.
	 */
	public boolean addLabelByTag(Object tag, String textLabel) {
		if (isUnLimitedLables || !checkLabelsCompleted()) {
			Label label = new Label(getContext(), mTextSize, mIconCross, mShowCross, mTextColor, mBackgroundResource, mLabelsClickables, mLabelPadding);
			label.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
			label.setText(textLabel);
	...
	public void onClickCross(View view) {
		...
		if (listenerOnRemoveLabel != null) {
			if (view.getTag() instanceof Integer) {
				...
				listenerOnRemoveLabel.onRemoveLabel(view.getTag(), -1);
			}
		}
	 ...
	 *
	 * @param labelToRemove
	 *            the text of the {@link Label} to remove.
	 */
	 ...
	 *
	 * @param tag
	 *            the tag of the {@link Label} to remove.
	 *
	 * @param labelToRemove
	 *            the text of the {@link Label} to remove.
	 */
	public boolean removeLabelByTag(Object tag, String labelToRemove) {
		Label view = (Label) findViewWithTag(tag);
		if (view != null) {
			removeView(view);
			decreaseLabelsCounter();
			if (getLabelsCounter() == EMPTY) {
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		if (state instanceof Bundle) {
			...
			List<LabelValues> labelsAdded = (List<LabelValues>) bundle.getSerializable("labelsAdded");
	...
	/**
	 * Interface for a callback when a label is removed. Container
	 * Activity/Fragment must implement this interface
	 */
	public interface OnRemoveLabelListener {
		 ...
		 *            of the item to remove.
		 */
		void onRemoveLabel(Object view, int position);
	}

	/**
	 * Interface for a callback listener when there are the maximum number of
	 * labels. Container Activity/Fragment must implement this interface
	 */
	...
	/**
	 * Interface for a callback listener when there are not labels. Container
	 * Activity/Fragment must implement this interface
	 */
