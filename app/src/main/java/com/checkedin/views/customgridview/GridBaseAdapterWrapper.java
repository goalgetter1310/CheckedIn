...
public class GridBaseAdapterWrapper extends BaseAdapter {
	...
	private final GridBaseAdapter mDelegate;
	...
	public GridBaseAdapterWrapper(Context context, GridView gridView,
			GridBaseAdapter delegate) {
		mContext = context;
		mDelegate = delegate;
		mGridView = gridView;
	 ...
	 * 
	 * @param position
	 *            Position of the item whose data we want within the adapter_timeline_friend's
	 *            data set.
	...
	/**
	 * @return the adapter_timeline_friend wrapped by this adapter_timeline_friend.
	 */
	public GridBaseAdapter getWrappedAdapter() {
		return mDelegate;
	}
	...
	protected class HeaderFillerView extends FrameLayout {
		...
		/**
		 * Set the adapter_timeline_friend id for this header so we can easily pull it later.
		 */
