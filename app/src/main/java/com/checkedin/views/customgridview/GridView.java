...
public class GridView extends android.widget.GridView implements
	...
	static final String TAG = GridView.class.getSimpleName();
	...
	protected GridBaseAdapterWrapper mAdapter;
	...
	public void setAdapter(ListAdapter adapter) {
		...
		GridBaseAdapter baseAdapter = null;
		if (adapter instanceof GridBaseAdapter) {
			baseAdapter = (GridBaseAdapter) adapter;
		} else if (adapter instanceof GridSimpleAdapter) {
			// Wrap up simple adapter_timeline_friend to auto-generate the data we need.
			baseAdapter = new GridSimpleAdapterWrapper(
					(GridSimpleAdapter) adapter);
		}

		this.mAdapter = new GridBaseAdapterWrapper(getContext(), this,
				baseAdapter);
	...
	private int findMotionHeader(float y) {
		...
		for (int i = getFirstVisiblePosition(); i <= getLastVisiblePosition();) {
			...
			if (id == GridBaseAdapterWrapper.ID_HEADER) {
	...
	private void scrollChanged(int firstVisibleItem) {
		...
		if (childCount != 0) {
			...
			for (int i = 0; i < childCount; i += mNumMeasuredColumns) {
				...
				if (mAdapter.getItemId(getPositionForView(child)) == GridBaseAdapterWrapper.ID_HEADER
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		for (int i = 0; i < headerPositions.size(); i++) {
			...
			boolean headerIsStickied = ((GridBaseAdapterWrapper.HeaderFillerView) frame).getHeaderId() == mCurrentHeaderId
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// Update adapter_timeline_friend with number of columns.
		if (mAdapter != null) {
