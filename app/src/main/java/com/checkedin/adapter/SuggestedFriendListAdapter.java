...
import com.checkedin.fragment.FriendSuggestionFrg;
...
public class SuggestedFriendListAdapter extends RecyclerView.Adapter<SuggestedFriendListAdapter.ViewHolder> implements BubbleTextGetter {
    private Activity activity;
    private int resource;
    private ArrayList<User> items;
    private FriendSuggestionFrg friendListFrg;

    public SuggestedFriendListAdapter(Activity activity, int resource, ArrayList<User> items, FriendSuggestionFrg friendListFrg) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        holder.tvFriendName.setText(items.get(position).getFirstName()+" "+items.get(position).getLastName());
        String checkinDate = items.get(position).getvLastCheckin();
        String checkinPlace = items.get(position).getvLastCheckinLocation();
        ...
        Utility.loadImageGlide(holder.civProfilePic,items.get(position).getImageUrl());
//                Picasso.with(activity).load(items.get(position).getThumbImage()).error(R.drawable.ic_placeholder).into(holder.civProfilePic);
    ...
    public String getTextToShowInBubble(int position) {
        String fullName = items.get(position).getFirstName()+" "+items.get(position).getLastName();
