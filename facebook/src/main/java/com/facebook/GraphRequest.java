 ...
 * user. In the former case, no AccessToken needs to be specified, while in the
 * latter, an AccessToken must be provided. If requests are executed in a batch,
 * a Facebook application ID must be associated with the batch, either by
 * setting the application ID in the AndroidManifest.xml or via FacebookSdk or
 * by calling the {@link #setDefaultBatchApplicationId(String)
 * setDefaultBatchApplicationId} method.
 * </p>
 * <p>
 * After completion of a request, the AccessToken, if not null and taken from
 * AccessTokenManager, will be checked to determine if its Facebook access token
 * needs to be extended; if so, a request to extend it will be issued in the
 * background.
 * </p>
 */
public class GraphRequest {
	...
	/**
	 * Constructs a request without an access token, graph path, or any other
	 * parameters.
	 */
	public GraphRequest() {
	...
	/**
	 * Constructs a request with an access token to retrieve a particular graph
	 * path. An access token need not be provided, in which case the request is
	...
	/**
	 * Constructs a request with a specific AccessToken, graph path, parameters,
	 * and HTTP method. An access token need not be provided, in which case the
	 * request is sent without an access token and thus is not executed in the
	 * context of any particular user. Only certain graph requests can be
	 * expected to succeed in this case.
	 * <p/>
	 ...
	 */
	public GraphRequest(AccessToken accessToken, String graphPath,
			Bundle parameters, HttpMethod httpMethod, Callback callback) {
	...
	public static GraphRequest newDeleteObjectRequest(AccessToken accessToken,
			String id, Callback callback) {
		return new GraphRequest(accessToken, id, null, HttpMethod.DELETE,
				callback);
	}
	 ...
	 *            the graph path to retrieve, create, or delete
	 * @param graphObject
	 *            the graph object to create or update
	 * @param callback
	 *            a callback that will be called when the request is completed
	 *            to handle success or error conditions
	 * @return a Request that is ready to execute
	...
	public static GraphRequest newPostRequest(AccessToken accessToken,
			String graphPath, JSONObject graphObject, Callback callback) {
		GraphRequest request = new GraphRequest(accessToken, graphPath, null,
		...
		Callback wrapper = new Callback() {
			...
			public void onCompleted(GraphResponse response) {
				if (callback != null) {
					...
					JSONArray data = result != null ? result
							.optJSONArray("data") : null;
	...
	public static GraphRequest newPlacesSearchRequest(AccessToken accessToken,
			...
			String searchText, final GraphJSONArrayCallback callback) {
		...
		return new GraphRequest(accessToken, SEARCH, parameters,
				HttpMethod.GET, wrapper);
	}

	/**
	 * Creates a new Request configured to upload a photo to the specified graph
	 * path.
	 *
	 * @param accessToken
	 *            the access token to use, or null
	 * @param graphPath
	 *            the graph path to use, defaults to me/photos
	 * @param image
	 *            the bitmap image to upload
	 * @param caption
	 *            the user generated caption for the photo, can be null
	 * @param params
	 *            the parameters, can be null
	 * @param callback
	 *            a callback that will be called when the request is completed
	 *            to handle success or error conditions, can be null
	 * @return a Request that is ready to execute
	 */
	public static GraphRequest newUploadPhotoRequest(AccessToken accessToken,
			String graphPath, Bitmap image, String caption, Bundle params,
			Callback callback) {
		...
		return new GraphRequest(accessToken, graphPath, parameters,
				HttpMethod.POST, callback);
	}

	/**
	 * Creates a new Request configured to upload a photo to the specified graph
	 * path. The photo will be read from the specified file.
	 *
	 * @param accessToken
	 *            the access token to use, or null
	 * @param graphPath
	 ...
	 */
	public static GraphRequest newUploadPhotoRequest(AccessToken accessToken,
			String graphPath, File file, String caption, Bundle params,
	...
	/**
	 * Creates a new Request configured to upload a photo to the specified graph
	 * path. The photo will be read from the specified Uri.
	 *
	 * @param accessToken
	 *            the access token to use, or null
	 * @param graphPath
	 ...
	 */
	public static GraphRequest newUploadPhotoRequest(AccessToken accessToken,
			String graphPath, Uri photoUri, String caption, Bundle params,
			Callback callback) throws FileNotFoundException {
		graphPath = getDefaultPhotoPathIfNull(graphPath);
		if (Utility.isFileUri(photoUri)) {
			return newUploadPhotoRequest(accessToken, graphPath, new File(
					photoUri.getPath()), caption, params, callback);
	 ...
	 * <p/>
	 * The GraphObject in the response will include a
	 ...
	 * way: if the specified access token (or active access token if `null`) is
	 * valid, the ID will represent the user associated with the active access
	 * token; otherwise the ID will represent the user logged into the native
	 * Facebook app on the device. A `null` ID will be provided into the
	 * callback if a) there is no native Facebook app, b) no one is logged into
	 * it, or c) the app has previously called
	 * {@link FacebookSdk#setLimitEventAndDataUsage(android.content.Context, boolean)}
	 * ;} with `true` for this user. <b>You must call this method from a
	 * background thread for it to work properly.</b>
	 *
	 * @param accessToken
	 *            the access token to issue the Request on, or null If there is
	 *            no logged-in Facebook user, null is the expected choice.
	 * @param context
	 ...
	 *            be thrown.
	 * @param applicationId
	 *            explicitly specified Facebook App ID. If null, the application
	 *            ID from the access token will be used, if any; if not, the
	 *            application ID from metadata will be used.
	 * @param callback
	 *            a callback that will be called when the request is completed
	...
	public static GraphRequest newCustomAudienceThirdPartyIdRequest(
			AccessToken accessToken, Context context, String applicationId,
			Callback callback) {
		...
		if (accessToken == null) {
			// Only use the attributionID if we don't have an access token. If
			// we do, then the user
			// token will be used to identify the user, and is more reliable
			// than the attributionID.
		...
		return new GraphRequest(accessToken, endpoint, parameters,
	 ...
	 */
	public static GraphRequest newCustomAudienceThirdPartyIdRequest(
			AccessToken accessToken, Context context, Callback callback) {
		return newCustomAudienceThirdPartyIdRequest(accessToken, context, null,
				callback);
	}
	 ...
	 *
	 * @return the GraphObject associated with this request, or null if there is
	 *         none
	 */
	 ...
	 *
	 * @return the access token associated with this request, or null if none
	 *         has been specified
	 */
	 ...
	 *
	 * @return the name of this requests batch entry, or null if none has been
	 *         specified
	 */
	 ...
	 *            the name of this requests entry in a batched request, which
	 ...
	 *
	 * @return the name of this requests dependency, or null if none has been
	 *         specified
	 */
	 ...
	 * batched requests. Batched requests require an application ID, so either
	 * at least one request in a batch must provide an access token or the
	 * application ID must be specified explicitly.
	 *
	 ...
	 * requests if none of those requests specifies an access token. Batched
	 * requests require an application ID, so either at least one request in a
	 * batch must specify an access token or the application ID must be
	...
	public final void setCallback(final Callback callback) {
		...
		if (FacebookSdk
				.isLoggingBehaviorEnabled(LoggingBehavior.GRAPH_API_DEBUG_INFO)
				|| FacebookSdk
						.isLoggingBehaviorEnabled(LoggingBehavior.GRAPH_API_DEBUG_WARNING)) {
			Callback wrapper = new Callback() {
				@Override
				public void onCompleted(GraphResponse response) {
					JSONObject responseObject = response.getJSONObject();
					JSONObject debug = responseObject != null ? responseObject
							.optJSONObject(DEBUG_KEY) : null;
					JSONArray debugMessages = debug != null ? debug
							.optJSONArray(DEBUG_MESSAGES_KEY) : null;
					if (debugMessages != null) {
						for (int i = 0; i < debugMessages.length(); ++i) {
							JSONObject debugMessageObject = debugMessages
									.optJSONObject(i);
							String debugMessage = debugMessageObject != null ? debugMessageObject
									.optString(DEBUG_MESSAGE_KEY) : null;
							String debugMessageType = debugMessageObject != null ? debugMessageObject
									.optString(DEBUG_MESSAGE_TYPE_KEY) : null;
							String debugMessageLink = debugMessageObject != null ? debugMessageObject
									.optString(DEBUG_MESSAGE_LINK_KEY) : null;
	...
	/**
	 * Executes this request on the current thread and blocks while waiting for
	 * the response.
	 * <p/>
	...
	/**
	 * Executes the request asynchronously. This function will return
	 * immediately, and the request will be processed on a separate thread. In
	 * order to process result of a request, or determine whether a request
	 * succeeded or failed, a callback must be specified (see the
	...
	public static HttpURLConnection toHttpConnection(GraphRequestBatch requests) {
		...
		try {
			if (requests.size() == 1) {
				...
				// request URLs are
				// serialized as relative_url parameters within each batch
				// entry.
	...
	/**
	 * Executes a single request on the current thread and blocks while waiting
	 * for the response.
	 * <p/>
	...
	/**
	 * Executes requests on the current thread as a single batch and blocks
	...
	/**
	 * Executes requests as a single batch on the current thread and blocks
	...
	public static List<GraphResponse> executeBatchAndWait(
		...
		try {
			...
			List<GraphResponse> responses = GraphResponse
					.constructErrorResponses(requests.getRequests(), null,
							new FacebookException(ex));
			runCallbacks(requests, responses);
			return responses;
		}

		List<GraphResponse> responses = executeConnectionAndWait(connection,
				requests);
		return responses;
	}
	 ...
	 */
	public static GraphRequestAsyncTask executeBatchAsync(
			GraphRequest... requests) {
		Validate.notNull(requests, "requests");
	...
	public static List<GraphResponse> executeConnectionAndWait(
		...
		return executeConnectionAndWait(connection, new GraphRequestBatch(
				requests));
	}
	...
	public static List<GraphResponse> executeConnectionAndWait(
		...
		List<GraphResponse> responses = GraphResponse.fromHttpConnection(
				connection, requests);
		...
		if (numRequests != responses.size()) {
			throw new FacebookException(String.format(Locale.US,
	...
	public static GraphRequestAsyncTask executeConnectionAsync(
		...
		GraphRequestAsyncTask asyncTask = new GraphRequestAsyncTask(connection,
				requests);
	...
	public String toString() {
		return new StringBuilder().append("{Request: ")
				...
				.append(", graphPath: ").append(graphPath)
	...
	static void runCallbacks(final GraphRequestBatch requests,
		...
		for (int i = 0; i < numRequests; ++i) {
			...
			if (request.callback != null) {
				callbacks.add(new Pair<Callback, GraphResponse>(
						request.callback, responses.get(i)));
			}
		}
		...
		if (callbacks.size() > 0) {
			Runnable runnable = new Runnable() {
				public void run() {
					...
					List<GraphRequestBatch.Callback> batchCallbacks = requests
							.getCallbacks();
	...
	private void addCommonParameters() {
		if (this.accessToken != null) {
			...
			if (!Utility.isNullOrEmpty(appID)
				...
				Log.d(TAG,
						"Warning: Request without access token missing application ID or"
								+ " client token.");
			}
		}
	...
	private void serializeToBatch(JSONArray batch,
		...
		for (String key : keys) {
			...
			if (isSupportedAttachmentType(value)) {
				...
				String name = String.format(Locale.ROOT, "%s%d",
			...
			processGraphObject(this.graphObject, relativeURL,
					new KeyValueSerializer() {
						...
						public void writeString(String key, String value)
								throws IOException {
							keysAndValues.add(String.format(Locale.US, "%s=%s",
	...
	private static void setConnectionContentType(HttpURLConnection connection,
	...
	final static boolean shouldWarnOnMissingFieldsParam(GraphRequest request) {
		...
		return versionParts.length >= 2
				...
				|| (Integer.parseInt(versionParts[0]) >= 2 && Integer
						.parseInt(versionParts[1]) >= 4);
	}

	final static void validateFieldsParamForGetRequests(
			GraphRequestBatch requests) {
		...
		for (GraphRequest request : requests) {
			if (HttpMethod.GET.equals(request.getHttpMethod())
				...
				if (!params.containsKey(FIELDS_PARAM)
						|| Utility
								.isNullOrEmpty(params.getString(FIELDS_PARAM))) {
					Logger.log(LoggingBehavior.DEVELOPER_ERRORS, Log.WARN,
							"Request",
	...
	final static void serializeToUrlConnection(GraphRequestBatch requests,
		...
		try {
			...
			if (hasOnProgressCallbacks(requests)) {
				...
				processRequest(requests, null, numRequests, url,
						countingStream, shouldUseGzip);

				int max = countingStream.getMaxProgress();
				Map<GraphRequest, RequestProgress> progressMap = countingStream
						.getProgressMap();

				outputStream = new ProgressOutputStream(outputStream, requests,
						progressMap, max);
			}
	...
	private static void processRequest(GraphRequestBatch requests,
			Logger logger, int numRequests, URL url, OutputStream outputStream,
			boolean shouldUseGzip) throws IOException, JSONException {
	...
	private static void processGraphObject(JSONObject graphObject, String path,
		...
		// foolproof way of
		// knowing if we are posting an OG Action, given that batched requests
		// can have parameter
		// substitution, but passing the OG Action type as a substituted
		// parameter is unlikely.
	 ...
	 *
	 * Use this method to create an open graph object, which can then be posted
	 * utilizing the same GraphRequest methods as other GraphRequests.
	 *
	 * @param openGraphObject
	 *            The open graph object to create. Only SharePhotos with the
	 *            imageUrl set are accepted through this helper method.
	 * @return GraphRequest for creating the given openGraphObject
	 * @throws FacebookException
	 *             thrown in the case of a JSONException or in the case of
	 *             invalid format for SharePhoto (missing imageUrl)
	 */

	public static GraphRequest createOpenGraphObject(
			final ShareOpenGraphObject openGraphObject)
			throws FacebookException {
		...
		try {
			JSONObject stagedObject = (JSONObject) OpenGraphJSONUtility
					.toJSONValue(openGraphObject,
			...
			String graphPath = String.format(Locale.ROOT, GRAPH_PATH_FORMAT,
					ME, "objects/" + ogType);
			return new GraphRequest(AccessToken.getCurrentAccessToken(),
					graphPath, parameters, HttpMethod.POST);
		} catch (JSONException e) {
	...
	private static void processGraphObjectProperty(String key, Object value,
		...
		if (JSONObject.class.isAssignableFrom(valueClass)) {
			...
			// formats Facebook
			// accepts and returns. Apps created after 8/1/12 (or apps that have
			// explicitly enabled
			// the migration) should send/receive dates in ISO-8601 format.
			// Pre-migration apps can
			// send as Unix timestamps. Since the future is ISO-8601, that is
			// what we support here.
			// Apps that need pre-migration behavior can explicitly send these
	...
	private static void serializeParameters(Bundle bundle,
			Serializer serializer, GraphRequest request) throws IOException {
	...
	private static String getUserAgent() {
		if (userAgent == null) {
			...
			if (!Utility.isNullOrEmpty(customUserAgent)) {
				userAgent = String.format(Locale.ROOT, "%s/%s", userAgent,
						customUserAgent);
			}
		}
	...
	private static class Serializer implements KeyValueSerializer {
		...
		public void writeObject(String key, Object value, GraphRequest request)
				throws IOException {
		...
		public void writeRequestsAsJson(String key, JSONArray requestJsonArray,
				Collection<GraphRequest> requests) throws IOException,
				JSONException {
			if (!(outputStream instanceof RequestOutputStream)) {
		...
		public void writeContentUri(String key, Uri contentUri, String mimeType)
			...
			InputStream inputStream = FacebookSdk.getApplicationContext()
			...
			if (outputStream instanceof ProgressNoopOutputStream) {
				...
				((ProgressNoopOutputStream) outputStream)
						.addProgress(contentSize);
			} else {
				totalBytes += Utility.copyAndCloseInputStream(inputStream,
						outputStream);
			}
		...
		public void writeFile(String key, ParcelFileDescriptor descriptor,
			...
			if (outputStream instanceof ProgressNoopOutputStream) {
				...
				ParcelFileDescriptor.AutoCloseInputStream inputStream = new ParcelFileDescriptor.AutoCloseInputStream(
						descriptor);
				totalBytes += Utility.copyAndCloseInputStream(inputStream,
						outputStream);
			}
		...
		public void write(String format, Object... args) throws IOException {
			if (!useUrlEncode) {
				...
				this.outputStream.write(URLEncoder.encode(
						String.format(Locale.US, format, args), "UTF-8")
						.getBytes());
			}
		}
	 ...
	 * {@link FacebookSdk#setOnProgressThreshold(long)}
	 */
	...
	public interface GraphJSONObjectCallback {
		 ...
		 *
		 * @param object
		 *            the GraphObject representing the returned object, or null
		 * @param response
		 *            the Response of this request, which may include error
		 *            information if the request was unsuccessful
		 */
	...
	public static class ParcelableResourceWithMimeType<RESOURCE extends Parcelable>
		...
		private ParcelableResourceWithMimeType(Parcel in) {
			...
			resource = in.readParcelable(FacebookSdk.getApplicationContext()
					.getClassLoader());
		}
	}
}
