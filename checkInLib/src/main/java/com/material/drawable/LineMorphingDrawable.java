...
import com.material.util.ViewUtil;
...
public class LineMorphingDrawable extends Drawable implements Animatable {
	...
	private LineMorphingDrawable(State[] states, int curState, int paddingLeft,
			int paddingTop, int paddingRight, int paddingBottom,
			int animDuration, Interpolator interpolator, int strokeSize,
			int strokeColor, Paint.Cap strokeCap, Paint.Join strokeJoin,
			boolean clockwise) {
		mStates = states;
	...
	public void draw(Canvas canvas) {
		...
		float degrees = (mClockwise ? 180 : -180)
				* ((mPrevState < mCurState ? 0f : 1f) + mAnimProgress);
	...
	private void updatePath() {
		...
		if (mAnimProgress == 0f
				|| (mStates[mPrevState].links != null && mAnimProgress < 0.05f))
			updatePathWithState(mPath, mStates[mPrevState]);
		else if (mAnimProgress == 1f
				|| (mStates[mCurState].links != null && mAnimProgress > 0.95f))
			updatePathWithState(mPath, mStates[mCurState]);
		else
			updatePathBetweenStates(mPath, mStates[mPrevState],
					mStates[mCurState],
					mInterpolator.getInterpolation(mAnimProgress));
	...
	private void updatePathWithState(Path path, State state) {
		if (state.links != null) {
			...
			for (int i = 0, count = state.points.length / 4; i < count; i++) {
				...
				path.moveTo(getX(state.points[index]),
						getY(state.points[index + 1]));
				path.lineTo(getX(state.points[index + 2]),
						getY(state.points[index + 3]));
			}
		} else {
			for (int i = 0, count = state.points.length / 4; i < count; i++) {
				int index = i * 4;
	...
	private void updatePathBetweenStates(Path path, State prev, State cur,
			float progress) {
		...
		for (int i = 0; i < count; i++) {
			...
			mPath.moveTo(getX(x1 + (x3 - x1) * progress), getY(y1 + (y3 - y1)
					* progress));
			mPath.lineTo(getX(x2 + (x4 - x2) * progress), getY(y2 + (y4 - y2)
					* progress));
		}
	}
	...
	private void update() {
		...
		float value = Math.min(1f, (float) (curTime - mStartTime)
				/ mAnimDuration);
	...
	public static class Builder {
		...
		public Builder(Context context, AttributeSet attrs, int defStyleAttr,
			...
			TypedArray a = context
					.obtainStyledAttributes(attrs,
							R.styleable.LineMorphingDrawable, defStyleAttr,
							defStyleRes);
			int resId;

			if ((resId = a.getResourceId(
					R.styleable.LineMorphingDrawable_lmd_state, 0)) != 0)
				states(readStates(context, resId));
			curState(a.getInteger(
					R.styleable.LineMorphingDrawable_lmd_curState, 0));
			padding(a.getDimensionPixelSize(
					R.styleable.LineMorphingDrawable_lmd_padding, 0));
			paddingLeft(a.getDimensionPixelSize(
					R.styleable.LineMorphingDrawable_lmd_paddingLeft,
					mPaddingLeft));
			...
			paddingBottom(a.getDimensionPixelSize(
					R.styleable.LineMorphingDrawable_lmd_paddingBottom,
					mPaddingBottom));
			animDuration(a.getInteger(
					R.styleable.LineMorphingDrawable_lmd_animDuration,
					context.getResources().getInteger(
							android.R.integer.config_mediumAnimTime)));
			if ((resId = a.getResourceId(
					R.styleable.LineMorphingDrawable_lmd_interpolator, 0)) != 0)
				interpolator(AnimationUtils.loadInterpolator(context, resId));
			strokeSize(a.getDimensionPixelSize(
					R.styleable.LineMorphingDrawable_lmd_strokeSize,
					ThemeUtil.dpToPx(context, 3)));
			strokeColor(a.getColor(
					R.styleable.LineMorphingDrawable_lmd_strokeColor,
					0xFFFFFFFF));
			int cap = a.getInteger(
					R.styleable.LineMorphingDrawable_lmd_strokeCap, 0);
			if (cap == 0)
			...
			int join = a.getInteger(
					R.styleable.LineMorphingDrawable_lmd_strokeJoin, 0);
			if (join == 0)
			...
			clockwise(a.getBoolean(
					R.styleable.LineMorphingDrawable_lmd_clockwise, true));
			...
			try {
				...
				do {
					if (eventType == XmlPullParser.START_TAG) {
						...
						throw new RuntimeException("Expecting menu, got "
								+ tagName);
					}
					eventType = parser.next();
					...
					case XmlPullParser.END_TAG:
						...
						case TAG_POINTS:
							...
							for (int i = 0; i < state.points.length; i++)
								state.points[i] = Float
										.parseFloat(array.get(i));
							break;
						case TAG_LINKS:
		...
		public LineMorphingDrawable build() {
			...
			return new LineMorphingDrawable(mStates, mCurState, mPaddingLeft,
					mPaddingTop, mPaddingRight, mPaddingBottom, mAnimDuration,
					mInterpolator, mStrokeSize, mStrokeColor, mStrokeCap,
					mStrokeJoin, mClockwise);
		}
